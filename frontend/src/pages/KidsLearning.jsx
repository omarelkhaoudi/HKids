...
function KidsLearning() {
 ...
 <Link
 to="/kids"
 className="grid h-14 w-14 place-items-center rounded-full bg-card text-emerald-700 shadow-md"
 aria-label="Accueil"
 >
 ...
 className="grid h-14 w-14 place-items-center rounded-full bg-card text-foreground-500 shadow-md"
 ...
 <div key={category.id} className="grid min-h-24 min-w-24 place-items-center rounded-[1.75rem] bg-card/20 p-3">
 ...
 {user?.role !== 'kid' && (
 <section className="mb-6 rounded-[1.5rem] bg-card p-4 shadow-lg">
 ...
 {selectedContent ? (
 <section className="rounded-[2rem] bg-card p-5 shadow-xl">
 <button
 onClick={() => setSelectedContent(null)}
 className="mb-4 inline-flex h-14 items-center gap-2 rounded-2xl bg-surface-secondary px-4 font-black"
 >
 ...
 <div key={question.id} className="rounded-[1.5rem] bg-surface-secondary p-4">
 ...
 active ? 'border-emerald-500 bg-emerald-50' : 'border-white bg-card'
}`}
 ...
 {loading ? (
 <div className="rounded-[2rem] bg-card p-8 text-center font-black shadow-lg">...</div>
 ) : (
 ...
 >
 <span className="grid h-20 w-20 place-items-center rounded-[1.5rem] bg-card/25 text-5xl">
 {content.category_pictogram}
 </span>
 <span className="mt-5 block text-2xl font-black">{content.title}</span>
 <span className="mt-2 inline-flex rounded-full bg-card/20 px-3 py-1 text-sm font-black">
 {content.reward?.icon || '⭐'}
 </span>
 </motion.button>
 ))}
 ...
 <div key={challenge.id} className="rounded-[1.5rem] bg-card p-4 shadow-lg">
 ...
 <div className="mt-3 h-3 overflow-hidden rounded-full bg-surface-secondary">
 <div
