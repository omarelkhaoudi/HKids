...
function KidsHome() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fbff] text-surface-900 overflow-x-hidden font-sans pb-32">
      
      {/* HEADER */}
      <header className="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-xl border-b border-surface-200 shadow-sm px-4 py-3 flex items-center justify-between">
        <Link to="/kids" className="shrink-0 transition-transform hover:scale-105 active:scale-95">
          <Logo size="default" showText={true} />
        </Link>
        <div className="flex items-center gap-2 md:gap-4">
          <div className="hidden md:flex items-center gap-4 bg-surface-100 rounded-full px-4 py-2">
          ...
          <button onClick={handleLogout} className="p-3 bg-white border border-surface-200 rounded-full text-primary-500 shadow-sm hover:bg-surface-50 hover:scale-105 transition-all">
      ...
      <div className="relative pt-24 pb-12 px-4 sm:px-8 lg:px-12 bg-gradient-to-b from-sky-300 via-sky-200 to-[#f8fbff] overflow-hidden">
        ...
        <motion.div animate={{ x: [0, 50, 0], y: [0, -20, 0] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} className="absolute top-10 left-10 w-64 h-64 bg-white/40 rounded-full blur-3xl" />
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center gap-8">
          ...
          >
            ...
            <div className="absolute -bottom-4 -right-4 bg-white rounded-full p-2 shadow-xl z-20">
          ...
          <div className="text-center md:text-left flex-1">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 bg-white/60 backdrop-blur-md px-4 py-2 rounded-full font-bold text-primary-700 mb-4 shadow-sm border border-white">
              <SparklesIcon className="w-5 h-5" /> Explorateur Stellaire
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl md:text-6xl font-black text-surface-900 tracking-tight mb-2">
              {greeting}, <span className="text-primary-600">{kidName}</span> !
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg md:text-xl font-bold text-surface-700 opacity-90">
              Prêt pour une nouvelle aventure aujourd'hui ?
            </motion.p>
          </div>

          {/* Quick Stats Mobile */}
          <div className="md:hidden flex items-center gap-4 bg-white/80 backdrop-blur-md rounded-3xl p-4 shadow-lg w-full justify-center">
             <div className="flex flex-col items-center"><div className="text-2xl">⭐</div><div className="font-black text-surface-900">{totalStars}</div></div>
             <div className="w-px h-10 bg-surface-200"></div>
             <div className="flex flex-col items-center"><div className="text-2xl">🔥</div><div className="font-black text-surface-900">{currentStreak}</div></div>
             <div className="w-px h-10 bg-surface-200"></div>
             <div className="flex flex-col items-center"><div className="text-2xl">🏆</div><div className="font-black text-surface-900">12</div></div>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 -mt-8 relative z-20 space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="h-full p-0 overflow-hidden border-4 border-white shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white relative group cursor-pointer" onClick={() => navigate(`/book/${continueReadingBook.id}`)}>
               ...
               <div className="p-6 md:p-8 flex flex-col md:flex-row gap-6 h-full items-center">
                 ...
                 <div className="flex-1 text-center md:text-left flex flex-col h-full justify-center">
                   <Badge variant="glass" className="bg-white/20 text-white border-none self-center md:self-start mb-3 font-bold">Reprendre la lecture</Badge>
                   ...
                   <div className="mt-auto w-full">
                     ...
                     <ProgressBar progress={continueReadingBook.progress} color="bg-green-400" className="h-3 bg-white/20" />
                   </div>
                 </div>
                 <div className="hidden md:flex items-center justify-center">
                   <button className="w-16 h-16 bg-white text-violet-600 rounded-full flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform">
          ...
          <motion.div whileHover={{ y: -5 }}>
            <Card className="h-full p-6 md:p-8 border-4 border-white shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] bg-white flex flex-col items-center justify-center text-center relative overflow-hidden">
              <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-amber-400 to-orange-500"></div>
              <h3 className="text-xl font-black text-surface-900 mb-2">Mission du jour</h3>
              <p className="text-surface-500 font-bold mb-6">Lis pendant 15 minutes</p>
        ...
        <section>
          <h2 className="text-2xl font-black text-surface-900 mb-6 px-2">Créer & Jouer</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link to="/kids/story-studio" className="group p-6 bg-gradient-to-br from-primary-400 to-primary-600 rounded-[2rem] text-white shadow-lg hover:shadow-xl hover:-translate-y-2 transition-all flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden">
              ...
              <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-12 transition-transform"><SparklesIcon className="w-8 h-8" /></div>
            ...
            <Link to="/kids/ai-stories" className="group p-6 bg-gradient-to-br from-secondary-400 to-secondary-600 rounded-[2rem] text-white shadow-lg hover:shadow-xl hover:-translate-y-2 transition-all flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
              <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:-rotate-12 transition-transform"><BookIcon className="w-8 h-8" /></div>
            ...
            <Link to="/content-library" className="group p-6 bg-gradient-to-br from-accent-400 to-accent-600 rounded-[2rem] text-white shadow-lg hover:shadow-xl hover:-translate-y-2 transition-all flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
               <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-12 transition-transform"><AudioIcon className="w-8 h-8" /></div>
            ...
            <Link to="/kids/learning" className="group p-6 bg-gradient-to-br from-green-400 to-green-600 rounded-[2rem] text-white shadow-lg hover:shadow-xl hover:-translate-y-2 transition-all flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
               <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:-rotate-12 transition-transform"><BrainIcon className="w-8 h-8" /></div>
        ...
        <section>
          <div className="flex items-center justify-between px-2 mb-6">
            <h2 className="text-2xl font-black text-surface-900">Recommandé pour toi</h2>
            <Link to="/content-library" className="text-primary-600 font-bold hover:text-primary-700">Voir tout</Link>
          </div>
        ...
        <section>
          <div className="flex items-center justify-between px-2 mb-6">
            <h2 className="text-2xl font-black text-surface-900">Nouvelles Histoires</h2>
          </div>
        ...
        {/* ACHIEVEMENTS & BADGES */}
        <section>
          <h2 className="text-2xl font-black text-surface-900 mb-6 px-2">Mes Trophées</h2>
          <Card className="p-8 border-4 border-white shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] bg-white">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {badges.map(badge => (
                <motion.div whileHover={{ scale: 1.05 }} key={badge.id} className="flex flex-col items-center text-center gap-3">
                  <div className={`w-24 h-24 rounded-full flex items-center justify-center text-4xl shadow-xl border-4 ${badge.unlocked ? 'bg-gradient-to-br from-yellow-300 to-amber-500 border-yellow-200' : 'bg-surface-100 border-surface-200 grayscale opacity-50'}`}>
                    {badge.unlocked ? badge.icon : <LockIcon className="w-8 h-8 text-surface-400" />}
                  </div>
                  <span className={`font-black ${badge.unlocked ? 'text-surface-900' : 'text-surface-400'}`}>{badge.name}</span>
                </motion.div>
              ))}
            </div>
