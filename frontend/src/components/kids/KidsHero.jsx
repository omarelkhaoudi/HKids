...
export function KidsHero({
  ...
  return (
    <motion.section
      ...
      className={`relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br ${gradient} p-8 md:p-12 text-white shadow-kids-soft border-4 border-white/40 ${className}`}
    >
      <div className="absolute -right-16 -top-16 w-72 h-72 bg-white/15 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -left-12 -bottom-12 w-56 h-56 bg-accent-300/20 rounded-full blur-3xl pointer-events-none" />
      {emoji && (
