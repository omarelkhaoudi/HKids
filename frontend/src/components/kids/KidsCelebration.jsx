...
/**
 * Lightweight celebration overlay (<2s default). Does not change business flow.
 */
export const KidsCelebration = memo(function KidsCelebration({
  ...
  return (
    <AnimatePresence>
      {active && (
        <motion.div
          className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-black/35 backdrop-blur-sm"
        ...
        >
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl font-black text-primary-600 mb-2">
              {title || t('kidBookDone')}
            </h2>
            {subtitle && (
              <p className="text-lg font-bold text-foreground-secondary mb-8">{subtitle}</p>
            )}
            <div className="flex flex-col gap-3">
              {primaryLabel && onPrimary && (
                <KidsButton onClick={onPrimary} className="!min-h-[56px] !w-full !text-lg" tone="primary">
                  {primaryLabel}
                </KidsButton>
              )}
