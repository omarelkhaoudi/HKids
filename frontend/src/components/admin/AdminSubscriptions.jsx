...
function AdminSubscriptions() {
  ...
  if (loading) {
    return <div className="p-8 text-center text-neutral-500">Chargement des abonnements...</div>;
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <h1 className="mt-1 text-3xl font-black text-neutral-900">Abonnements</h1>
        <p className="mt-2 text-neutral-500">Consultation des plans et abonnements actifs.</p>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <AdminMetricCard label="Gratuit" value={summary.free_subscribers || 0} detail="parents sans abonnement actif" icon={UserIcon} tone="bg-neutral-100 text-neutral-600" />
        ...
        <AdminMetricCard label="Actifs" value={summary.active_subscriptions || 0} detail="abonnements actifs" icon={ClockIcon} tone="bg-red-50 text-red-600" />
      </div>

      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-red-100 bg-white/90 p-5 shadow-lg">
          <h2 className="text-lg font-black text-neutral-900">Plans disponibles</h2>
          <div className="mt-4 grid grid-cols-1 gap-3">
            {(data?.plans || []).map((plan) => (
              <div key={plan.id} className="rounded-xl bg-neutral-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black text-neutral-900">{plan.name}</p>
                    <p className="mt-1 text-sm text-neutral-500">{plan.description || 'Sans description'}</p>
                  </div>
                ...
                <p className="mt-2 text-xs font-bold text-neutral-500">{plan.book_limit} livre(s) par periode</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-red-100 bg-white/90 p-5 shadow-lg">
          <h2 className="text-lg font-black text-neutral-900">Abonnements actifs</h2>
          <div className="mt-4 space-y-3">
            {(data?.active_subscriptions || []).length > 0 ? data.active_subscriptions.map((subscription) => (
              <div key={subscription.id} className="rounded-xl bg-neutral-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black text-neutral-900">{subscription.parent_name}</p>
                    <p className="text-sm text-neutral-500">{subscription.plan_name}</p>
                  </div>
      ...
      <section className="rounded-2xl border border-red-100 bg-white/90 p-5 shadow-lg">
        <h2 className="text-lg font-black text-neutral-900">Fonctionnalites futures preparees</h2>
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
          {[
            ...
            <div key={title} className="rounded-xl bg-red-50 p-4">
