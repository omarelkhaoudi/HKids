  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-surface-900 tracking-tight">Quiz & Jeux</h1>
          <p className="text-surface-500 font-medium mt-1">Gérez le contenu ludo-éducatif (Coming Soon).</p>
        </div>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium text-surface-500">{item.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4"><Badge variant="soft" className="bg-surface-100 text-surface-700 font-bold capitalize">{item.type}</Badge></td>
                  <td className="p-4">
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 bg-surface-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl">
              <h3 className="text-xl font-black mb-4">Fonctionnalité en développement</h3>
              <p className="text-surface-500 mb-6">L'éditeur complet de quiz et jeux éducatifs sera disponible dans la prochaine mise à jour.</p>
