  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-white via-primary-50/40 to-secondary-50/20 dark:from-surface-900 dark:via-primary-900/20 dark:to-secondary-900/10 pt-12 pb-16 md:pt-20 md:pb-24">
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        {Array.from({ length: reducedMotion ? 6 : 15 }).map((_, i) => (
          reducedMotion ? (
            <div
              key={i}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-start justify-center lg:justify-start gap-4 mb-space-12">
              <div className="w-full sm:w-auto flex flex-col items-center lg:items-start gap-2">
                <Button
                  size="lg"
                  className="rounded-full shadow-lg w-full sm:w-auto min-h-[3.25rem]"
                  onClick={() => navigate('/parent/signup')}
                >
                  {t.startFree}
                </Button>
                <p className="text-xs text-foreground-muted font-medium px-2">{t.landingHeroPrimaryHint}</p>
              </div>
              <div className="w-full sm:w-auto flex flex-col items-center lg:items-start gap-2">
                <Button
                  variant="ghost"
                  size="lg"
                  className="rounded-full bg-white dark:bg-surface-800 border border-border shadow-md w-full sm:w-auto min-h-[3.25rem]"
                  onClick={() => navigate('/stories')}
                >
                  {t.viewDemo}
                </Button>
                <p className="text-xs text-foreground-muted font-medium px-2">{t.landingHeroDemoHint}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 text-sm font-semibold text-foreground-muted">
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-full bg-secondary-100 flex items-center justify-center text-secondary-600" aria-hidden="true">✓</div>
                {t.trustSecure}
              </div>
              <div className="flex items-center gap-2">
          ...
          <motion.div
            initial={reducedMotion ? false : { opacity: 0, scale: 0.95, y: 40 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ duration: 0.8, delay: reducedMotion ? 0 : 0.2, ease: 'easeOut' }}
            className="relative mx-auto w-full max-w-lg lg:max-w-none"
          >
            <div className="relative aspect-square w-full">
              <div className="absolute inset-10 bg-gradient-to-tr from-primary-400 via-secondary-300 to-primary-300 rounded-[3rem] blur-3xl opacity-40" aria-hidden="true" />
              ...
              <div className="absolute inset-0 bg-white/40 dark:bg-surface-800/40 backdrop-blur-2xl rounded-[2.5rem] border border-white/60 dark:border-white/10 shadow-floating flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-white/80 to-white/10 dark:from-white/10 dark:to-transparent z-0" aria-hidden="true" />
                ...
                <div className="relative z-10 w-full h-full flex flex-col items-center justify-center p-8">
                  ...
                  <div className="w-full max-w-[280px] aspect-square rounded-[2rem] bg-gradient-to-b from-primary-50 to-white shadow-2xl border-4 border-white relative mt-8 flex flex-col items-center justify-end overflow-hidden group">
                    <div className="absolute inset-0 flex items-center justify-center p-2">
                      <img src="/enfant3ans.webp" alt="" className="w-full h-full object-cover rounded-[1.5rem] filter drop-shadow-xl group-hover:scale-105 transition-transform duration-500 motion-reduce:transform-none" />
                    </div>
                ...
                <div className="absolute top-6 right-6 bg-white/90 dark:bg-surface-800/90 backdrop-blur-md rounded-2xl px-5 py-3 shadow-lg border border-white/50 text-center z-20 max-w-[9rem]">
                  <div className="text-sm font-extrabold text-primary-600 leading-snug">{catalogLabel}</div>
                </div>

                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white/90 dark:bg-surface-800/90 backdrop-blur-md rounded-full px-5 py-3 shadow-lg border border-white/50 flex items-center gap-3 w-max max-w-[calc(100%-2rem)] z-20">
